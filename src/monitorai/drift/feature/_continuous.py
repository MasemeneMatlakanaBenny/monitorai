...
from monitorai._urls._drift_urls import batch_drift_feature
from monitorai._pydantic import BatchFeatureDriftSDKRequest
from monitorai._exceptions._excepts import (
    AuthenticationError,
    BadRequestError,
    ConflictError,
    NetworkError,
...
class ContinuousFeatureDrift:
    def __init__(self,
                 client:MonitorAI,
                 ref_data:pd.DataFrame,
                 analysis_data:pd.DataFrame):
        """
        The ContinuousFeatureDrift class is used to calculate drift for features between the reference and analysis datasets.
        Args:
            client (MonitorAI):
    ...
    def _response(self,feature:str):
        try:
            drift_request=BatchFeatureDriftSDKRequest(ref_data=self.ref_data,
    ...
    def total_variation_dist(self,feature:str):
        """
        Args:
            feature (str) : the feature compute drift for using the total variation distance.
    ...
    def wasserstein_dist(self,feature:str):
        """
        Args:
            feature (str) : the feature compute drift for using the wasserstein distance.

        Returns:
            float : the wasserstein distance.
        """
        return self._response(feature)["wasserstein_dist"]
